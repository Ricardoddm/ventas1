...
class VendorsController{
    public async listVendors(req: Request, res: Response ){
        ...
        const vendor = await db.query('SELECT * FROM vendedor');
    ...
    public async getVendor(req: Request, res: Response): Promise<any>{
        const { id } = req.params;
        const vendor = await db.query('SELECT * FROM vendors WHERE id_vendedor = ?', [id]);
        console.log(vendor);
    ...
    public async createVendor(req: Request, res: Response){
        ...
        const sales = db.query('INSERT INTO vendedor set ?', [req.body]);
        res.json(sales);
    }
    ...
    public async updateVendor(req: Request, res: Response){
        ...
        await db.query('UPDATE vendedor SET ? WHERE id_vendedor = ?', [saleInfo, id]);
